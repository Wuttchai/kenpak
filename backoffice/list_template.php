...
if($_GET[method]=="del"){
	...
	mysqli_query($conn, $sql);
	mysqli_close($conn);
	header('Location: list_template.php?page='.$_GET[page]);
}
?>
<!DOCTYPE html>
                                ...
                                <div class="portlet light bordered">
                                    <div class="portlet-title">
                                        <div class="caption font-dark">
											...
											<a href="add_template.php" class="btn btn-circle green-jungle btn-sm"><span class="glyphicon glyphicon-plus"></span> Add Template Orders List. </a> 
									...
									<input type="hidden" name="page" value="1">
										...
										<?php
											...
											$sql = "SELECT * FROM fs_template 
													WHERE 1=1 AND Customer_Id like '$_SESSION[user_id]'";
											if($_GET[text]!=""){
											...
											<table class="table table-striped table-bordered table-hover">
												...
												?>
													<tr>
														...
														<td>
															<a href="edit_detailtemplate.php?id=<?=$row[Template_Id]?>" class="btn btn-circle btn-xs blue-chambray"><i class="icon-list"></i> Set Details </a> 
															<a href="cart_template.php?id=<?=$row[Template_Id]?>&cid=<?=$_SESSION[user_id];?>" class="btn btn-circle btn-xs blue-chambray"><i class="icon-list"></i> Order by Tempate </a> 
															| 
															<a href="edit_template.php?id=<?=$row[Template_Id]?>" class="btn btn-circle btn-xs blue"><i class="glyphicon glyphicon-pencil"></i> Edit </a>
															<a href="list_template.php?page=<?=$_GET[page]?>&id=<?=$row[Template_Id]?>&method=del" class="btn btn-circle btn-xs red" onclick="return confirm('Are you sure you want to delete this item?');"><i class="glyphicon glyphicon-trash"></i> Delete </a>
														</td>
													</tr>
												<?php
														$i++;
