...
if($_POST[method]=="1"){
	...
	if(mysqli_num_rows($query_check)>0){
		...
		$sql_chk = "SELECT * FROM fs_template_detail WHERE Template_Id = '$_POST[Template_Id]' AND SellProduct_Id = '$get_row[SellProduct_Id]'";
		...
		if($rowcount==0){
			...
			mysqli_close($conn);
			header('Location: edit_detailtemplate.php?id='.$_POST[Template_Id]);
		}else{
...
if($_GET[method]=="del"){
	...
	mysqli_query($conn, $sql);
	mysqli_close($conn);
	header('Location: edit_detailtemplate.php?id='.$_GET[id]);
}

$sql = "SELECT * FROM fs_template WHERE Template_Id = '$_GET[id]' AND Customer_Id like '$_SESSION[user_id]'";
						...
						<div class="row">
                            <div class="col-md-12">
														...
														<tr>
															...
															<?php 
																$sql_in = "SELECT * FROM fs_template_detail 
																		...
																		WHERE fs_product.ProductCategory_Code = '$row_cat[ProductCategory_Code]' AND fs_template_detail.Template_Id = '$_GET[id]' AND fs_template.Customer_Id like '$_SESSION[user_id]'";
															...
															?>
																<tr>
																	...
																	<td>
																		...
																		<a href="edit_detailtemplate.php?id=<?=$_GET[id]?>&proid=<?=$row_in[SellProduct_Id]?>&method=del" class="btn btn-circle btn-xs red" onclick="return confirm('Are you sure you want to delete this item?');"><i class="glyphicon glyphicon-trash"></i> Delete </a>
																	</td>
																</tr>
															<?php
																	$i++;
								...
								<?php
									...
									echo "<center> ไม่พบ Template ที่คุณต้องการค้นหา</center>";
								}
								?>
